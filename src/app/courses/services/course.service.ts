...
export class CourseService {
  ...
  loadCourses(title?: string): Observable<Course[]> {
    return this.getCourses(title).pipe(
      tap((courses: Course[]) => this.courseSubject.next(courses))
    );
  }
  ...
  private getCourses(title?: string): Observable<Course[]> {
    return this.courseApiService.getCourses(title);
  }
  ...
  postCourse(course: Course) {
    return this.courseApiService.postCourse(course).pipe(
      tap(() => {
