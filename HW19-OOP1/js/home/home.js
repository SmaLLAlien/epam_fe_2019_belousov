...
import {SliderTestMonial, SliderLatestPortfolio} from './slider2.js';
...
function makeSlidersWork() {
  ...
  const testMonials = new SliderTestMonial(testimonials__content, 1);
  const lastPortfolio = new SliderLatestPortfolio(containerSlider1, 3);
  testMonials.addListeners();
  testMonials.makeInfinity();
  lastPortfolio.addListeners();
  lastPortfolio.makeInfinity();
  lastPortfolio.swipe();
  lastPortfolio.makeChangeSlide();
  testMonials.makeChangeSlide();

  lastPortfolio.makeShadow();
  testMonials.expandDescription();
  testMonials.showPreview();
}
