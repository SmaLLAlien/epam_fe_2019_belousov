function Slider(container, elementsToShow) {// eslint-disable-line max-statements
  ...
  this.makeInfinity = () => {
    if (this.infinity) {
      this.timerId = setInterval(toRight.bind(self), 2500);
    }
  };
  ...
  this.getToRight = () => {
    return toRight.call(self);
  };
  this.getToLeft = () => {
    return toLeft.call(self);
  };
...
export function SliderTestMonial(container, elementsToShow) {
  ...
  this.showPreview = () => {
    this.controlRight.addEventListener('mouseover', showNextPreview.bind(this));
  };
